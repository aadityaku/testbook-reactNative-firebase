...
const CourseCart = ({value,navigation}) => {
  
  return(
    <View style={{width:"100%",backgroundColor:"white",paddingHorizontal:2,borderRadius:5,paddingVertical:4,marginTop:10}}>
    <View style={{justifyContent:"space-between",flexDirection:"row",marginTop:2,paddingHorizontal:4,backgroundColor:"white"}}>
           <Text style={{fontSize:18,color:"black",fontWeight:"600"}}>{value.title}</Text>
           <TouchableOpacity style={{backgroundColor:"blue",alignItems:"center",padding:4,marginTop:3,borderRadius:6}} onPress={() => navigation.navigate('ViewCourse')}>
    ...
    <View style={{paddingHorizontal:7,paddingVertical:5}}>
     {
      courseCategory.map((item,index) => (
        <CourseCart key={index} value={item} navigation={navigation}/>
      ))
     }
    </View>
  )
